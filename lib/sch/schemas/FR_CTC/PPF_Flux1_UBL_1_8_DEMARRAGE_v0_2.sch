...
<schema xmlns="http://purl.oclc.org/dsdl/schematron" queryBinding="xslt2" schemaVersion="ISO19757-3">
  <title>PPF — Flux 1 UBL e-invoicing &amp; e-reporting — Profil de Base (Trajectoire DÉMARRAGE)</title>
  ...
  <!-- Patched locally (#86171), diverges from AIFE's published artifact.
       Defects: $totalTVA bound to cac:TaxTotal/cbc:TaxAmount is a 2-node
       sequence on non-EUR extracts (G1.12 requires BT-111 in EUR), so
       number() raises XPTY0004 and Saxon produces no SVRL; float comparison
       rejects the 1-cent gap the rule text tolerates. Fix: qualify binding
       to ventilated TaxTotal, scope TVA assert to EUR documents (Annexe 7:
       "applicable uniquement si la devise ... est l'EURO"), compare in
       xs:decimal, skip non-castable amounts (G1.14's job) instead of dying.
       Drop when AIFE republishes corrected schematron. -->
  <pattern id="F1-START-TOTAL-COHERENCE-G1.53">
    <rule context="/ubl-invoice:Invoice | /ubl-creditnote:CreditNote">
      ...
      <let name="totalTVA" value="cac:TaxTotal[cac:TaxSubtotal]/cbc:TaxAmount"/>
      <let name="sommeTVA" value="cac:TaxTotal/cac:TaxSubtotal/cbc:TaxAmount"/>
      <assert test="not($totalHT castable as xs:decimal) or not($sommeBasesHT) or not(every $m in $sommeBasesHT satisfies $m castable as xs:decimal) or (abs(xs:decimal($totalHT) - sum(for $m in $sommeBasesHT return xs:decimal($m))) &lt;= 0.01)" flag="fatal">[G1.53] Écart sur le Total Hors Taxe global (<value-of select="$totalHT"/>) vis-à-vis de la somme des bases ventilées (<value-of select="sum(for $m in $sommeBasesHT return xs:decimal($m))"/>). Tolérance de 0,01 EUR. | Source : Annexe 7 v1.8 G1.53</assert>
      <assert test="not(cbc:DocumentCurrencyCode = 'EUR') or not($totalTVA castable as xs:decimal) or not($sommeTVA) or not(every $m in $sommeTVA satisfies $m castable as xs:decimal) or (abs(xs:decimal($totalTVA) - sum(for $m in $sommeTVA return xs:decimal($m))) &lt;= 0.01)" flag="fatal">[G1.53] Écart sur le Montant Total de TVA global (<value-of select="$totalTVA"/>) vis-à-vis de la somme des lignes de TVA calculées (<value-of select="sum(for $m in $sommeTVA return xs:decimal($m))"/>). Tolérance de 0,01 EUR. | Source : Annexe 7 v1.8 G1.53</assert>
    </rule>
  </pattern>
